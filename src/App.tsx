...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] text-slate-800 flex flex-col font-sans selection:bg-amber-100 antialiased">
      ...
      {/* MODAL: FIREBASE GOOGLE SIGN IN DIALOG OVERLAY */}
      {showLoginModal && (
        <div 
          className="fixed inset-0 bg-slate-900/50 backdrop-blur-[3px] flex items-center justify-center p-4 z-50 animate-in fade-in duration-200"
          onClick={() => setShowLoginModal(false)}
        >
          <div 
            className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 md:p-8 max-w-[400px] w-full relative animate-in zoom-in-95 duration-200 flex flex-col items-center text-center"
            onClick={(e) => e.stopPropagation()}
          ...
          >
            ...
            <div className="flex justify-center select-none mt-2 mb-3 text-[26px] font-semibold tracking-[-0.03em]">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 mb-6 leading-relaxed">
              Use your Google Account to authenticate directly with Firebase Authentication and access the Data Science BookShelf.
            </p>

            {loginError && (
              <div className="w-full mb-5 p-3 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/60 rounded-xl text-xs text-red-600 dark:text-red-400 flex items-start gap-2 text-left">
            ...
            <button 
              ...
              onClick={async () => {
                ...
                setIsSigningIn(true);
                try {
                  console.log('[App UI] Triggering Google sign-in popup...');
                  await signInWithGoogle();
                  setShowLoginModal(false);
                } catch (err: any) {
                  console.error('[App UI] Google sign-in failed:', err);
                  let msg = err?.message || 'Google sign-in failed. Please try again.';
                  if (err?.code === 'auth/popup-blocked') {
                    msg = 'The sign-in popup was blocked by your browser. Please allow popups for this site and try again.';
                  } else if (err?.code === 'auth/popup-closed-by-user') {
                    msg = 'The sign-in popup was closed before completing authentication. Please try again.';
                  } else if (err?.code === 'auth/unauthorized-domain') {
                    msg = 'This domain is not authorized for Google sign-in in Firebase Console. Add your Cloud Run domain to Authorized Domains under Firebase Authentication settings.';
                  } else if (err?.code === 'auth/operation-not-allowed') {
                    msg = 'Google sign-in is not enabled under Firebase Authentication → Sign-in method in Firebase Console.';
                  } else if (err?.code === 'auth/network-request-failed') {
                    msg = 'Network error during sign-in. Please check your internet connection.';
                  }
                  setLoginError(msg);
                } finally {
                  setIsSigningIn(false);
              ...
              className="w-full py-3 px-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-700 font-semibold text-sm rounded-xl transition-all shadow-xs hover:shadow-md flex items-center justify-center gap-3 cursor-pointer active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSigningIn ? (
                <>
                  <Loader2 className="w-5 h-5 text-amber-600 animate-spin" />
                  <span>Opening Google Popup...</span>
                </>
              ) : (
                <>
            ...
            <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-normal text-center mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
              Authenticates securely via Firebase Auth. No app-specific password required.
            </p>
          </div>
        </div>
      )}
